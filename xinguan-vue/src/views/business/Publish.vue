                ...
                <el-col :span="12">
                  <div class="grid-content bg-purple">
                    <el-select v-if="existence === 1" style="width:100%;" filterable
                               @change="consumerSelectChange(addRuleForm.consumerId)" v-model="addRuleForm.consumerId"
                               placeholder="选择已存在去向">
              ...
              <div v-if="existence === 1">
                <el-card class="box-card" style="margin-bottom: 30px;">
                  <div class="text item" style="font-size: 12px;padding: 5px;">
                    详细地址: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{ consumerInfo.name }}</el-tag></span>
                  </div>
                  <div class="text item" style="font-size: 12px;padding: 5px;">
                    区县信息: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{ consumerInfo.address }}</el-tag></span>
                  </div>
                  <div class="text item" style="font-size: 12px;padding: 5px;">
                    联系方式: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{ consumerInfo.phone }}</el-tag></span>
                  </div>
                  <div class="text item" style="font-size: 12px;padding: 5px;">
                    联系人员: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{ consumerInfo.contact }}</el-tag></span>
                  </div>
                </el-card>
              </div>
              <el-row>
          ...
          <el-card id="card2">
            ...
            <el-form size="mini" :inline="true" :model="queryMap" class="demo-form-inline">
              <el-form-item label="分类">
                <el-cascader :change-on-select="true" @change="selectChange" v-model="categorykeys" :props="selectProps" :options="catetorys" clearable></el-cascader>
              </el-form-item>
              <el-form-item>
                <el-form-item label="名称">
                  <el-input clearable v-model="queryMap.name" placeholder="名称" @clear="search(1)"></el-input>
                </el-form-item>
                <el-button type="primary" @click="search(1)">查询</el-button>
              </el-form-item>
            </el-form>
            <el-alert title="勾选先下方物资后,在左侧表单的明细中添加其发放数量" type="warning" show-icon style="margin-bottom:20px;" :closable="false"></el-alert>
            <!-- 发放预选表格 -->
            <el-table size="mini" style="width: 100%" :data="page.records" border :row-key="getRowKey" ref="dataTable" @selection-change="handleSelectionChange" :max-height="this.getTableMaxHeight(0.44)">
              ...
              <el-table-column align="center" prop="imageUrl" label="图片">
                <template slot-scope="scope">
                  <el-popover placement="right" trigger="hover">
                    ...
                    <img slot="reference" :src="scope.row.imageUrl" alt="alt" style="height: 32px;width: 32px;cursor: pointer">
                  </el-popover>
                </template>
              </el-table-column>
              <el-table-column align="center" prop="name" :show-overflow-tooltip='true' label="名称"></el-table-column>
              <el-table-column align="center" prop="pnum" label="商品编号" :show-overflow-tooltip='true'></el-table-column>
              <el-table-column align="center" prop="model" label="型号"></el-table-column>
              <el-table-column align="center" prop="stock" label="库存">
                <template slot-scope="scope">
                  <el-tag type="primary" effect="plain" v-text="scope.row.stock"></el-tag>
                </template>
              </el-table-column>
            ...
            <el-drawer size="49%" title="发放明细" :visible.sync="drawer" :with-header="false">
              <span>
                ...
                <el-table size="mini" :data="products" border style="margin: 15px">
                  ...
                  <el-table-column align="center" prop="model" label="型号"></el-table-column>
                  <el-table-column align="center" label="数量" width="160">
                    <template slot-scope="scope">
                      <el-input-number @change="outStockNumberChange(scope.row.number, scope.row.stock)" size="mini" v-model="scope.row.number" :min="1" :max="scope.row.stock" label="描述文字"></el-input-number>
                    </template>
                  </el-table-column>
                  <el-table-column align="center" prop="unit" label="单位"></el-table-column>
                  <el-table-column align="center" label="操作" fixed="right">
                    <template slot-scope="scope">
                      <el-button type="danger" size="mini" plain icon="el-icon-remove" @click="removeItem(scope.row.pnum)">移除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              </span>
...
import { findProductStocks } from '@/api/product'
import { addOutStock } from '@/api/out-stock'
...
export default {
  ...
  methods: {
    ...
    getTableData (page = this.pageNum) {
      findProductStocks(page, this.pageSize, this.categorys, this.queryMap).then(res => {
        ...
        if (this.page.records.length === 0) {
          this.infoNotify = this.$notify.warning({
            title: '温馨提示',
            message: '当前暂时没有商品可供发放，请即使补充库存！',
            duration: 15000
          })
        }
      }).catch().finally()
    ...
    consumerSelectChange (consumerId) {
      this.consumerInfo = this.consumers.find(function (x) {
      ...
      this.$refs.addRuleFormRef.validate(async valid => {
        if (valid) {
          ...
          if (car.length > 0) {
            ...
            return this.$message.warning('发放商品的数量不能为空，请勾选物资后在明细中添加数量！')
          }
          const res = await this.$confirm('此操作将提交发放单, 是否继续?', '提示', {
          ...
          if (res === 'confirm') {
            addOutStock(this.addRuleForm).then(() => {
              this.$confirm('物资发放单已进入审核状态，是否继续添加物资发放单信息?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
              }).then(() => {
                ...
                this.$router.push('/business/product/out-stocks')
              })
            }).catch().finally()
          }
        }
